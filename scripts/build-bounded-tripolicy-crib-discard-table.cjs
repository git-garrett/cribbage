...
async function main() {
  ...
  const outDir = path.resolve(root, args.outDir || path.join("benchmarks", "crib-discard", `bounded-tripolicy-crib-discard-${dateSlug()}`));
  const outputPath = path.resolve(root, args.output || path.join(outDir, "crib-score-histogram-bounded-tripolicy-by-discard-cut.json"));
  ...
  const writeStatus = (status = "running") => {
    ...
    writeJsonAtomic(statusPath, {
      ...
      kind: "bounded-tripolicy-crib-discard-synthetic",
  ...
  try {
    await Promise.all(chunks.map((chunk, workerIndex) => runChunk({ chunk, checkpointDir, oldMb, memoLimit, workerIndex, tradeoffRatio })));
  } finally {
  ...
  const summary = assemble({ roots, checkpointDir, outputPath, startedAt, workerCount, oldMb, memoLimit, tradeoffRatio });
...
function printUsage() {
  console.log(`Usage: node scripts/build-bounded-tripolicy-crib-discard-table.cjs [options]

Builds a restartable sparse synthetic crib-discard table. For each perspective
role, own discard rank pair, and cut rank, it enumerates compatible opponent
six-card rank holdings and chooses the opponent discard under three policies:
  ev  = maximize hand EV +/- crib EV
  on  = deviate from EV only to gain own points at no more than 2:1 opponent cost
  off = deviate from EV only to suppress opponent points at no more than 2:1 own cost
...
function runChunk({ chunk, checkpointDir, oldMb, memoLimit, workerIndex, tradeoffRatio }) {
  return new Promise((resolve, reject) => {
    ...
    const worker = new Worker(__filename, {
      workerData: { chunk, checkpointDir, memoLimit, workerIndex, tradeoffRatio },
...
async function runWorker({ chunk, checkpointDir, memoLimit, workerIndex, tradeoffRatio }) {
  ...
  for (const job of chunk) {
    ...
    const startedAt = Date.now();
    const checkpoint = buildRootCheckpoint({ job, sixHands, choiceMemo, handEvMemo, cribEvMemo, availabilityMemo, rankScoreMemo, tradeoffRatio });
...
function buildRootCheckpoint({ job, sixHands, choiceMemo, handEvMemo, cribEvMemo, availabilityMemo, rankScoreMemo, tradeoffRatio }) {
  const cutCounts = emptyRanks();
  cutCounts[job.cut] = 1;
  ...
  const policies = { ev: serializePolicyMap(policyMaps.ev) };
  const sparseStats = {
    sourceEntries: 2,
    storedEntries: 0,
    omittedEntries: 0,
  ...
  for (const policy of ["on", "off"]) {
    const entry = serializePolicyMap(policyMaps[policy]);
    if (policyEntryEquivalent(entry, policies.ev)) {
      sparseStats[`${policy}Omitted`] += 1;
      sparseStats.omittedEntries += 1;
    } else {
      policies[policy] = entry;
      sparseStats[`${policy}Stored`] += 1;
      sparseStats.storedEntries += 1;
    }
  }
  return {
    version: 1,
...
function chooseDiscardsForHand({ hand, role, choiceMemo, handEvMemo, cribEvMemo, availabilityMemo, rankScoreMemo, tradeoffRatio }) {
  ...
  const ev = chooseBest(candidates, (entry) => [entry.netEv, entry.direct.own, -entry.direct.opponent, -discardTieValue(entry.discard)]);
  const result = {
    ev,
    on: chooseBoundedDiscard(candidates, ev, "on", tradeoffRatio),
    off: chooseBoundedDiscard(candidates, ev, "off", tradeoffRatio),
  };
...
function chooseBoundedDiscard(candidates, ev, mode, tradeoffRatio) {
  const acceptable = candidates.filter((entry) => {
    if (entry === ev) return false;
    if (mode === "on") {
      const ownGain = entry.direct.own - ev.direct.own;
      const opponentCost = entry.direct.opponent - ev.direct.opponent;
      return ownGain > 0 && opponentCost <= tradeoffRatio * ownGain;
    }
    const opponentSuppressed = ev.direct.opponent - entry.direct.opponent;
    const ownCost = ev.direct.own - entry.direct.own;
    return opponentSuppressed > 0 && ownCost <= tradeoffRatio * opponentSuppressed;
  });
  if (!acceptable.length) return ev;
  return chooseBest(acceptable, (entry) => mode === "on"
    ? [entry.direct.own, entry.netEv, -entry.direct.opponent, -discardTieValue(entry.discard)]
    : [-entry.direct.opponent, entry.netEv, entry.direct.own, -discardTieValue(entry.discard)]);
}
...
function mergeSparseStats(target, source = {}) {
  for (const key of ["sourceEntries", "storedEntries", "omittedEntries", "onStored", "onOmitted", "offStored", "offOmitted"]) {
...
function addSparseRates(stats) {
  stats.onOmittedRate = (stats.onStored + stats.onOmitted) ? round(stats.onOmitted / (stats.onStored + stats.onOmitted), 6) : 0;
  stats.offOmittedRate = (stats.offStored + stats.offOmitted) ? round(stats.offOmitted / (stats.offStored + stats.offOmitted), 6) : 0;
...
function assemble({ roots, checkpointDir, outputPath, startedAt, workerCount, oldMb, memoLimit, tradeoffRatio }) {
  ...
  const artifact = {
    ...
    kind: "sparse synthetic rank-only crib discard table with EV plus bounded player-on/player-off opponent discard policy overrides",
    note: "Rank-only table excludes flushes and right jack; missing on/off policy entries intentionally fall back to EV because the bounded policy produced the same aggregate value.",
    policySemantics: {
      ev: "opponent discard maximizes rank-only hand EV plus own crib EV or minus opponent crib EV",
      on: `opponent discard deviates from EV only to gain own directly affected points at no more than ${tradeoffRatio}:1 opponent-point cost`,
      off: `opponent discard deviates from EV only to suppress opponent directly affected points at no more than ${tradeoffRatio}:1 own-point cost`,
    },
    ranks: RANKS,
    roots: roots.length,
    candidateHands,
