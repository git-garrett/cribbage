...
if (args.help) {
  console.log(`Usage: node scripts/pack-tripolicy-crib-discard-asset.cjs --input <json> --output <bin> [--manifest <json>]`);
  process.exit(0);
}

const inputPath = path.resolve(root, args.input || "web/src/models/schell_table-peg_table-14.0/crib-score-histogram-tripolicy-by-discard-cut.json");
...
for (const role of ["dealer", "pone"]) {
  for (const pairKey of pairKeys) {
    for (let cut = 0; cut < 13; cut += 1) {
      const entry = source.table?.[role]?.[pairKey]?.[cut] ?? null;
      for (const policy of POLICIES) {
        const policyEntry = entry?.policies?.[policy] ?? null;
...
const manifest = {
  ...
  binaryFormat: {
    ...
    entryIndex: "((roleIndex * pairCount + discardPairIndex) * 13 + cutRank) * 3 + policyIndex",
    roles: ["dealer", "pone"],
    policies: POLICIES,
  },
  pairKeys,
