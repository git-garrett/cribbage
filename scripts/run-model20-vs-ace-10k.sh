...
if [[ ! -f "$OUT_DIR/manifest.txt" ]]; then
  {
    ...
    printf 'candidate=%s\n' "$MODEL20"
    printf 'opponent=%s\n' "$ACE"
    printf 'experiment=model20-conditioned-beliefs-and-show-vs-ace1323\n'
    ...
    printf 'model20LivePegging=exhaustive-legal-information-wp-with-empirical-depletion\n'
...
run_orientation "20.0-left" "$MODEL20" "$ACE" &
candidate_pid=$!
run_orientation "13.23-left" "$ACE" "$MODEL20" &
opponent_pid=$!
