...
CORRECTION_ASSET="${ASSET_DIR}/model1322-corrections.bin"
...
if [[ "$WORKERS" -ne 6 ]]; then
  echo "This benchmark requires the measured optimum of six workers per orientation." >&2
  exit 1
fi
...
if [[ "$EXPECTED_CORRECTION_SHA256" != "unknown" && "$CORRECTION_SHA256" != "$EXPECTED_CORRECTION_SHA256" ]]; then
  echo "Model 13.22 correction checksum differs from the frozen job specification." >&2
  exit 1
fi
...
if [[ ! -f "$OUT_DIR/manifest.txt" ]]; then
  {
    ...
    printf 'candidate=%s\n' "$MODEL1322"
    printf 'opponent=%s\n' "$MODEL13215"
    printf 'experiment=model1322-sparse-correction-vs-model13215-board-matrix\n'
    printf 'model1322DiscardForecast=six-card-discard-sparse-correction\n'
    printf 'model1322OpeningLead=cut-indexed-pone-lead-mask\n'
    printf 'model1322LivePegging=executable-legal-information-model911-policy-with-owned-dead-cards\n'
    ...
    printf 'workerSelection=measured-12-core-paired-throughput-sweep\n'
    printf 'seed=%s\n' "$SEED"
    printf 'firstDealer=alternates-by-game-index\n'
    printf 'orientationPairing=same-seed-and-game-index-with-model-sides-swapped\n'
    printf 'crossBenchmarkPairing=same-seed-and-game-index-as-model13215-vs-model130\n'
...
run_orientation() {
  ...
  while read -r range_start range_games; do
    ...
    "$RUNNER" \
      ...
      --run-id "model1322-$label" \
      --matchup-id "13.22-vs-13.215" \
...
run_orientation "13.22-left" "$MODEL1322" "$MODEL13215" &
candidate_pid=$!
run_orientation "13.215-left" "$MODEL13215" "$MODEL1322" &
opponent_pid=$!
