...
node --no-warnings "$MODEL_ROOT/scripts/analyze-ai-run.cjs" \
  model1321-13.21-left \
  --db "$OUT_DIR/13.21-left/games.db" \
  --json > "$OUT_DIR/13.21-left-analysis.json"
node --no-warnings "$MODEL_ROOT/scripts/analyze-ai-run.cjs" \
  model1321-13.0-left \
  --db "$OUT_DIR/13.0-left/games.db" \
  --json > "$OUT_DIR/13.0-left-analysis.json"
python3 "$MODEL_ROOT/scripts/report_paired_benchmark.py" \
  --candidate-left-db "$OUT_DIR/13.21-left/games.db" \
...
node --no-warnings "$MODEL_ROOT/scripts/report-paired-live-benchmark.cjs" \
  ...
  --candidate "$MODEL1321" \
  --opponent "$MODEL130" \
  --candidate-left 13.21-left \
  --opponent-left 13.0-left \
  --candidate-left-run-id model1321-13.21-left \
  --opponent-left-run-id model1321-13.0-left \
