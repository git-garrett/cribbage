...
npm test --silent
scripts/run-quiet.sh "Production Ace integration" cargo test --manifest-path rust/Cargo.toml -p cribbage-api -- --ignored
scripts/run-quiet.sh "Full Ace asset integration" cargo test --manifest-path rust/Cargo.toml -p cribbage-shadow-engine --release model1323_full_asset_native_integration -- --ignored
