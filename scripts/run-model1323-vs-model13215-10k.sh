...
if [[ ! -f "$OUT_DIR/manifest.txt" ]]; then
  {
    ...
    printf 'candidate=%s\n' "$MODEL1323"
    printf 'opponent=%s\n' "$MODEL13215"
    printf 'experiment=model1323-joint-wp-vs-model13215-board-matrix\n'
    printf 'model1323DiscardForecast=verified-joint-distribution-board-wp\n'
    printf 'model1323OpeningLead=exhaustive-live-board-wp\n'
    printf 'model1323LivePegging=exhaustive-legal-information-wp-with-exact-pruning\n'
    ...
    printf 'workerSelection=prior-paired-benchmark-allocation-not-remeasured-for-1323\n'
    printf 'seed=%s\n' "$SEED"
...
run_orientation() {
  ...
  while read -r range_start range_games; do
    ...
    "$RUNNER" \
      ...
      --run-id "model1323-$label" \
      --matchup-id "13.23-vs-13.215" \
...
run_orientation "13.23-left" "$MODEL1323" "$MODEL13215" &
candidate_pid=$!
run_orientation "13.215-left" "$MODEL13215" "$MODEL1323" &
opponent_pid=$!
