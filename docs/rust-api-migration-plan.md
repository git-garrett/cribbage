...
- [x] Restore post-game human decision review in the Rust API with a fixed
  13.0 evaluator, saved decision-state snapshots, and explicit UI wording for
  win probability and point EV.
...
| 2026-07-16 | Public model reverted and deployed | At the user's direction, new public games and API fallback metadata use 13.0 for its substantial decision-speed advantage while the 13.0-vs-15.2 validation run continues. Deployed to production and verified `/health`, `/api/model`, and a fresh game all report `schell_table-peg_table-13.0`. Existing sessions retain their selected model. |
| 2026-07-17 | Post-game decision review restored and deployed | The native Rust API now saves every human discard and peg-play position, then evaluates it after the game with fixed 13.0. A local end-to-end game queued 8 discard and 32 pegging reviews; the endpoint completed all 40, returned 13.0 recommendations with point EV and win-probability values, and emptied both pending-review queues. Production QA then queued 40 reviews and confirmed the first completed review reported model 13.0 with both EV and win probability. The UI now identifies 13.0 explicitly and states that win probability is primary while point EV is supporting context. |
