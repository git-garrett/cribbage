...
- [ ] Persist whether each contested Model 16 decision used a learned policy
  entry or the legal-information fallback.
- [ ] Persist the selected entry confidence and selected policy weight without
  exposing hidden cards or changing live decisions.
- [ ] Add runner-only controls for learned-policy, fallback-only, deterministic
...
- [ ] Replace independent uniform board scores with a deterministic corpus of
  board states reached by actual non-clairvoyant engine games.
- [ ] Replace the trainer's simplified discard heuristic distribution with
