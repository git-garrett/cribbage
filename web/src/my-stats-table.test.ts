...
describe("myStatsTableRows", () => {
  it("builds one player-versus-AI comparison with scoring averages", () => {
    ...
    expect(rows).toContainEqual({ label: "Total scoring", player: "135", ai: "130", difference: "+5" });
    expect(rows).toContainEqual({ label: "Avg scoring", player: "13.50", ai: "13.00", difference: "+0.50" });
    expect(rows).toContainEqual({ label: "Pegging", player: "30", ai: "30", difference: "0" });
  });
  ...
  it("shows a dash when no detailed scoring games are available", () => {
    ...
    expect(row).toEqual({ label: "Avg scoring", player: "-", ai: "-", difference: "—" });
  });
});
